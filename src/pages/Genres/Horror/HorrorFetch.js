...
const HorrorFetch = ({ title, fetchUrl }) => {
   const [horror, setHorror] = useState(initialState);
   const [isLoading, setIsLoading] = useState(false);
   const [isLoadingMore, setIsLoadingMore] = useState(false);

   useEffect(() => {
      setIsLoading(i => !i);
      requests.fetchURL(fetchUrl, setHorror, 1);
   ...
   useEffect(() => {
      ...
      requests.fetchURL(fetchUrl, setHorror, horror.page + 1);

      setIsLoadingMore(false);
   }, [fetchUrl, isLoadingMore, horror.page]);
      ...
      <section className="row p-2">
         <div className="row__container container">
            ...
            <div className="row__poster">
               {horror.result.map(({ id, poster_path, title, name }, index) => {
                  return (
               ...
               {!horror.result ||
                  (Array.isArray(horror.result) &&
                     horror.result.length === 0 &&
                     Array.from(Array(20), (_, i) => <Skeleton key={i} />))}
            </div>
            {horror.page < horror.total_pages && !isLoading && (
               <button
