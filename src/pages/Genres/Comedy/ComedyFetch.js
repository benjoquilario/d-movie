...
const ComedyFetch = ({ title, fetchUrl }) => {
   const [comedy, setComedy] = useState(initialState);
   const [isLoading, setIsLoading] = useState(false);
   const [isLoadingMore, setIsLoadingMore] = useState(false);

   useEffect(() => {
      setIsLoading(i => !i);
      requests.fetchURL(fetchUrl, setComedy, 1);
   ...
   useEffect(() => {
      ...
      requests.fetchURL(fetchUrl, setComedy, comedy.page + 1);

      setIsLoadingMore(false);
   }, [fetchUrl, isLoadingMore, comedy.page]);
      ...
      <section className="row p-2">
         <div className="row__container container">
            ...
            <div className="row__poster">
               {comedy.result.map(({ id, poster_path, title, name }, index) => {
                  return (
               ...
               {!comedy.result ||
                  (Array.isArray(comedy.result) &&
                     comedy.result.length === 0 &&
                     Array.from(Array(20), (_, i) => <Skeleton key={i} />))}
            </div>
            {comedy.page < comedy.total_pages && !isLoading && (
               <button
