...
const RomanceFetch = ({ title, fetchUrl }) => {
   const [romance, setRomance] = useState(initialState);
   const [isLoading, setIsLoading] = useState(false);
   const [isLoadingMore, setIsLoadingMore] = useState(false);

   useEffect(() => {
      setIsLoading(i => !i);
      requests.fetchURL(fetchUrl, setRomance, 1);
   ...
   useEffect(() => {
      ...
      requests.fetchURL(fetchUrl, setRomance, romance.page + 1);

      setIsLoadingMore(false);
   }, [fetchUrl, isLoadingMore, romance.page]);
      ...
      <section className="row p-2">
         <div className="row__container container">
            ...
            <div className="row__poster">
               ...
               {!romance.result ||
                  (Array.isArray(romance.result) &&
                     romance.result.length === 0 &&
                     Array.from(Array(20), (_, i) => <Skeleton key={i} />))}
            </div>
            {romance.page < romance.total_pages && !isLoading && (
               <button
