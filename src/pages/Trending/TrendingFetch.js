...
const TrendingFetch = ({ title, fetchUrl }) => {
   ...
   useEffect(() => {
      const timer = setTimeout(() => {
         setIsLoading(i => !i);
         requests.fetchURL(fetchUrl, setTrending, 1);
   ...
   useEffect(() => {
      ...
      requests.fetchURL(fetchUrl, setTrending, trending.page + 1);

      setIsLoadingMore(false);
   }, [fetchUrl, isLoadingMore, trending.page]);
      ...
      <section className="row p-2">
         <div className="row__container container">
            ...
            <div className="row__poster">
               ...
               {!trending.result ||
                  (Array.isArray(trending.result) &&
                     trending.result.length === 0 &&
                     Array.from(Array(20), (_, i) => <Skeleton key={i} />))}
            </div>
            {trending.page < trending.total_pages && !isLoading && (
               <button
